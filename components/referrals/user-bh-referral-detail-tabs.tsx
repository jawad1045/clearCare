...
import { Eye, Edit, User, Paperclip, UserCheck, Calendar, FileText, Download, Activity } from "lucide-react";
  ...
  const statusHistory =
    rawHistory.length > 0
      ? rawHistory.map((entry: any) => ({
          ...entry,
          status: entry.status,
          date: entry.createdAt ?? entry.changedAt ?? entry.date ?? referral.lastUpdated ?? null,
